    ...
    for msg in history:
        ...
        if msg['role'] == 'user':
            ...
            for line in lines:
                ...
                # パターン1: 名前「セリフ」（例: 鏑木圭太朗「セリフ」、Keitaro Kaburagi「セリフ」）
                # パターン2: 「セリフ」のみ
                ...
                # パターン1を先にチェック（名前「セリフ」）
                # 日本語: 名前（全角スペース含む）「セリフ」
                # 英語: Name "dialogue" または Name「dialogue」
                name_dialogue_match = re.match(r'^(.+?)[「"]([^」"]+)[」"]$', p)
                if name_dialogue_match:
                    potential_name = name_dialogue_match.group(1).strip()
                    potential_dialogue = name_dialogue_match.group(2).strip()
                    # 名前が長すぎる（50文字以上）または句点を含む場合は地の文として扱う
                    if len(potential_name) <= 50 and "。" not in potential_name and "、" not in potential_name:
                        speaker_name_from_text = potential_name
                        dialogue_text = potential_dialogue
                        dialogue_match = True
...
def render_character_card(h, colors, is_active=False, show_debug=False):
    ...
    # 多言語対応のための言語マネージャーを初期化（最初に実行）
    from core.language import init_manager
    import os
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    lang_mgr = init_manager(BASE_DIR)
    ...
    if show_monitoring:
        monitoring_text = lang_mgr.get("text_0191", "脳内モニタ計測中…")
    ...
    def bar_row(label_key, v):
        label = lang_mgr.get(label_key, label_key)
    ...
    # 体験版: 名前の読み方を追加（鏑木圭太朗の場合、日本語のみ）
    name_display = name
    if IS_DEMO_MODE and name == "鏑木 圭太朗" and current_lang == "jp":
        name_display = f"{name}（{lang_mgr.get('text_0183', 'かぶらぎ けいたろう')}）"
    ...
    html += f'<div style="margin-top:6px;margin-bottom:8px;display:flex;align-items:center;gap:6px;font-size:13px;font-weight:800;color:#f5c542;text-shadow:0 1px 2px rgba(0,0,0,0.35);">💛 {lang_mgr.get("text_0184", "キズナ Lv.")}{bond}{rel_badge}</div>' \
            f'</div>' \
           f'{bar_row("text_0185", love)}' \
           f'{bar_row("text_0186", lust)}' \
           f'{bar_row("text_0187", possession)}' \
           f'{bar_row("text_0188", reason)}' \
           f'{bar_row("text_0189", chastity)}' \
           f'<div style="margin-top:12px;border-top:1px solid rgba(255,255,255,0.10);padding-top:12px;">' \
           f'<div style="font-size:14px;font-weight:800;color:{TEXT_SUB};margin-bottom:6px;">{lang_mgr.get("text_0190", "脳内モニタ（Top 5）")}</div>' \
           f'{chip_html}' \
           f'</div>' \
           f'</div>'
