...
def spin():
    ...
    jobs_data = load_json("male_jobs.json")
    pers_data = load_json("male_personalities.json")
    tones_data = load_json("male_tones.json")
    dial_data = load_json("male_dialects.json")
    # New assets
    vagina_data = load_json("male_eye_shapes.json")
    breast_data = load_json("male_body_traits.json")
    fetish_data = load_json("male_secrets.json")
    
    # Old assets fallback (if needed, though we use new ones primarily)
    bg_data = load_json("male_body_genitals.json")
    ...
    # Ensure "breasts" key is NOT used or handle missing key gracefully since male json doesn't have it
    # Male json only has 'body'. We can skip breast logic or fake it.
    # But wait, original code tries bg_data.get("breasts")
    # male_body_genitals.json ONLY has "body".
    # So breast logic below will fail if we don't adjust it.
    breast = {"desc": "なし", "tags": ""}
    
    
    # Secret Fetish Selection
    secret = {}
    if fetish_data and isinstance(fetish_data, list):
        secret = random.choice(fetish_data)
    ...
    character_data = {
        ...
        "secret_fetish": secret.get("name", "なし"),
        "secret_fetish_desc": secret.get("description", ""),
