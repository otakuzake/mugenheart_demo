...
STYLE_PREFIX = "score_9, score_8_up, score_7_up, source_anime, visual novel, japanese anime style, moe, ufotable, highly detailed anime CG, handsome male, flat color, cel shading, "

FIXED_POSITIVE_HEADER = """
(masterpiece, best quality:1.4), (official art:1.2), (absurdres, highres:1.2), <lora:more_details:0.5>, 
1boy, solo, handsome, ikemen, otome game style, detailed beautiful male face, detailed eyes, vivid colors, cinematic lighting, sparkling, bloom, depth of field, cool pose, 
"""
FIXED_POSITIVE_HEADER = FIXED_POSITIVE_HEADER.strip().replace("\n", " ")

R18_ADDITIONAL_TAGS = "(erotic:1.1), (soft erotic:1.2), shiny skin, blushing, slight sweat, defined muscles, vascular, abs, toned body, dynamic angle, "

FIXED_NEGATIVE_PROMPT = "(worst quality, low quality:1.4), (realistic, photorealistic, 3d, cosplay:1.3), lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, jpeg artifacts, signature, watermark, username, blurry, (1girl:1.5), female, breasts, feminine, girl, woman, mutated hands, bad proportions, simple background, flat color"

BASE_STANDING_PROMPT = "1boy, solo, cinematic lighting, detailed male face, cool pose"
BASE_INTIMATE_PROMPT = "female POV, (focus on boy:1.4), male solo, handsome male, detailed male face, upper body, romantic atmosphere, blushing, looking at viewer, detailed eyes"
BASE_MULTI_INTIMATE = "female POV, (focus on boys:1.4), two handsome boys, interacting with each other, close faces, romantic atmosphere, detailed male faces"
BASE_MULTI_NORMAL = "two boys, handsome, standing together, talking, casual atmosphere, school uniform or casual clothes"
...
def determine_fixed_status(client, user_input):
    ...
    # PhysicalTrait (筋肉・体格など) - 旧BreastTraitの代わり
    stats["PhysicalTraitId"] = "Normal"
    stats["PhysicalTraitDesc"] = "標準的な体格"
    
    # 女性用IDは空文字で埋める（エラー防止）
    stats["BreastTraitId"] = ""
    stats["BreastTraitDesc"] = ""
    stats["VaginaTraitId"] = ""
    stats["VaginaTraitDesc"] = ""
...
def send_to_comfyui(generated_tags, force_single=False, is_r18=False, is_both=False):
    ...
    """
    # 体験版モード: ComfyUIを使わず固定画像を返す
    from config import IS_DEMO_MODE
    if IS_DEMO_MODE:
        # demo_imagesフォルダからランダムに画像を選択
        demo_images_dir = os.path.join(BASE_DIR, "assets", "demo_images")
        demo_images = []
        if os.path.exists(demo_images_dir):
            for filename in os.listdir(demo_images_dir):
                if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                    demo_images.append(os.path.join(demo_images_dir, filename))
        ...
        if demo_images:
            ...
            try:
                ...
                print(f"Demo image load error: {e}")
                return {"status": "error", "message": f"Failed to load demo image: {e}"}
        else:
            # フォールバック: 空の画像データを返す
            return {"status": "error", "message": "No demo images found"}
        ...
        if "6" in prompt_workflow and "inputs" in prompt_workflow["6"]:
             ...
             if has_sub:
                 ...
                 if sub_h and hasattr(sub_h, "appearance"):
                     sub_add = f", second boy: ({sub_h.appearance}:1.5), detailed {sub_h.name}, completely different from first boy"
                     main_tags += sub_add + ", two completely distinct boys, no blending, separate identities"

             if has_sub:
                 final_tags = "two distinct boys with different features, " + main_tags
                 final_tags = final_tags.replace("1boy,", "two boys,").replace("solo,", "").replace("unknown hair,", "").replace("unknown eyes,", "").replace("unknown,", "")
             else:
             ...
             if has_sub:
                 ...
                 if main_h and hasattr(main_h, "appearance"):
                     main_prompt_part = f"first boy: ({main_h.appearance}:1.5), detailed {main_h.name}"
                 
                 sub_prompt_part = ""
                 if sub_h and hasattr(sub_h, "appearance"):
                     sub_prompt_part = f"second boy: ({sub_h.appearance}:1.5), detailed {sub_h.name}"
                 
                 character_details = f"two distinct boys, {main_prompt_part} BREAK {sub_prompt_part}, two separate characters, "
             else:
                 character_details = ""
             ...
             if main_h:
                 ...
                 if chas <= 20 and reas <= 20:
                     ...
                     # --- Context Aware Erotic Tags (Male Version) ---
                     current_hist = st.session_state.get("chat_history", [])
                     recent_text = " ".join([str(msg.get("parts", [""])[0]) for msg in current_hist[-5:]])
                     
                     # Base Erotic Tags
                     unlock_tags = ", topless male, bare chest, abs, muscular upper body, v-line, collarbone, sweaty glossy skin, aroused blushing face, heavy breathing"
                     
                     # Action: Hug
                     if any(k in recent_text for k in ["抱き", "ぎゅっ"]):
                         unlock_tags += ", hugging, embracing, strong arms"
                     
                     # Action: Kiss
                     if any(k in recent_text for k in ["キス", "唇", "接吻"]):
                         unlock_tags += ", kissing, close face, hand on cheek"

                     # Sexual Acts (Non-Explicit/Implied for R15)
                     if any(k in recent_text for k in ["手", "コキ", "撫で", "シゴ"]):
                         unlock_tags += ", hand on crotch, bulge, pleasure face"
                     elif any(k in recent_text for k in ["口", "舐め", "フェラ"]):
                         unlock_tags += ", head back, pleasure expression, hand in hair"
                     elif any(k in recent_text for k in ["挿入", "入", "中", "奥"]):
                         unlock_tags += ", bed scene, intimate embrace, sweaty bodies, top down view"
...
def generate_all_texts(client, input_data, status_data):
    ...
    Inner Desire: {status_data['Libido']}
    ...
    2. **Visual Detail (Green Box):** Describe her appearance.
       - **Perspective:** First Person ("Ore", "Boku", or "Watashi"). Let him speak in his own words.
       - Strictly reflect his "Tone" and personality.

    2. **Visual Detail (Green Box):** Describe his appearance.
       - **LENGTH:** **250 to 300 characters.**
       - **Requirement:** **MUST include his Height and Build (e.g., Tall, Muscular, Slender) explicitly.**
       - Describe hair, eyes, outfit, and body shape in detail.

    3. **Hidden Nature (Pink Box - The Secret):**
       - **LENGTH:** **Under 250 characters.**
       - **Perspective:** Third Person (Objective explanation/Analysis).
       - **Style:** Clear and descriptive.
       - **Task:** Explain his Hidden Side and hidden desires.
       - **R15 CONSTRAINT:** Describe his emotional intensity, and reactions vividly. Focus on his gap (e.g., cool on outside but beastly inside).

    4. **Image Prompt (English):** Visual tags only.
       - **Expression**: Include a specific facial expression tag.
       - **Tags**: Use "1boy", "handsome", "short hair", "male focus" etc.
