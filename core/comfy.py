import os
import sys

# ==========================================
# 📍 パス解決ロジック (内と外を使い分ける！)
# ==========================================
if getattr(sys, 'frozen', False):
    # 【EXE実行時】
    # 内部: EXEの中身（jsonなどの同梱ファイル）
    INTERNAL_DIR = sys._MEIPASS
    # 外部: EXEが置いてあるフォルダ（APIキーなど）
    EXTERNAL_DIR = os.path.dirname(sys.executable)
else:
    # 【開発時】どっちも同じ場所
    INTERNAL_DIR = os.path.dirname(os.path.abspath(__file__))
    EXTERNAL_DIR = INTERNAL_DIR

# ==========================================
# ⚙️ 設定値
# ==========================================

# ComfyUIのアドレス
# 標準のポート番号に戻す！
COMFYUI_SERVER_ADDRESS = "127.0.0.1:8188"
CLIENT_ID = "MugenHeartClient"

# ★重要★ ここを実際のファイル名「workflow_t2i_sfw.json」にする！
WORKFLOW_FILE = os.path.join(INTERNAL_DIR, "workflow_t2i_sfw.json")

# プロンプトの接頭辞
PROMPT_PREFIX = "masterpiece, best quality, highres, 1boy, male focus, handsome, otome game style, "

# APIキーは「EXEの外」にある！
API_KEY_PATH = os.path.join(EXTERNAL_DIR, "api_key.json")

# デバッグ表示
print(f"[CONFIG] INTERNAL: {INTERNAL_DIR}")
print(f"[CONFIG] WORKFLOW: {WORKFLOW_FILE}")