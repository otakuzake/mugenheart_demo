...
# ==========================================
# import sys, os は上でやってるから省略してもいいけど念の為
import sys
import os
...
lang_mgr.load_data("jp", "female_target") 
# ▲▲▲ 追加・移動 ▲▲▲

APP_VERSION = "1.5.0.CLEAN.r15"
IS_R18_APP = False

# models.py から移動した強力な解除命令
R18_BOOST_TEXT = """
【最優先指令：乙女ゲーム（運命の恋）モード】
1. **「運命」の演出:** あなたにとって、目の前の主人公は「まだ他人」ですが、**「本能的に惹かれてしまう運命の相手」**です。
   - 口では冷たくしたり、距離を取ったりしても、視線や態度には「気になって仕方がない」というニュアンスを滲ませてください。

2. **大人の距離感（重要）:**
   - **「親友」や「私生活」の話を、初対面の相手にペラペラ話さないでください。** それは不自然です。
   - ガードが高い状態では、あくまで「礼儀正しい他人」として振る舞い、拒絶するのではなく「微笑んでかわす」大人の余裕を見せてください。

3. **カレの魅力:** どんな状況でも、カレを「魅力的（イケメン）」に描くことを最優先してください。
"""
...
MODEL_OPTIONS = {
    # 3.0 Flash (最新・爆速)
    "⚡ 3.0 Flash Preview (推奨・爆速)": "models/gemini-3-flash-preview",
    
    # 3.0 Pro (最新・最強)
    "🧠 3.0 PRO Preview (最強の文章力)": "models/gemini-3-pro-preview",
    
    # 2.5 Flash (安定版バックアップ)
    "🐇 2.5 Flash Preview (安定・軽量)": "models/gemini-2.5-flash-preview-09-2025",
}
DEFAULT_MODEL_KEY = "⚡ 3.0 Flash Preview (推奨・爆速)"

# ★ここ重要！外側のフォルダ（EXTERNAL_DIR）を探すように変更！
KEY_FILE_PATH = os.path.join(EXTERNAL_DIR, "api_key.json")
...
if "gemini_api_key" not in st.session_state:
    ...
    try:
        if hasattr(st, "secrets") and "GEMINI_API_KEY" in st.secrets:
            dev_key = st.secrets["GEMINI_API_KEY"]
    except: pass

    if user_key:
        st.session_state.gemini_api_key = user_key
        st.session_state.gemini_model = settings.get("MODEL_NAME", MODEL_OPTIONS[DEFAULT_MODEL_KEY])
    elif dev_key:
        st.session_state.gemini_api_key = dev_key
        st.session_state.gemini_model = settings.get("MODEL_NAME", MODEL_OPTIONS[DEFAULT_MODEL_KEY])
    else:
        st.session_state.gemini_api_key = ""
        st.session_state.gemini_model = MODEL_OPTIONS[DEFAULT_MODEL_KEY]

# B. Gemini Client (ここで確定させる)
if "gemini_client" not in st.session_state:
    if st.session_state.gemini_api_key:
        try:
            model_name = st.session_state.get("gemini_model", MODEL_OPTIONS[DEFAULT_MODEL_KEY])
...
defaults = {
    ...
    "main_heroine": None,
    "chat_heroine": None,
    "chat_sub_heroine": None,
    "chat_history": [],
    "day_count": 1,
    "time_of_day": "夜",
    "location_text": "阿佐ヶ谷",
    # ユーザー指示: 場所の初期値を「月光荘」に変更（Dict構造は維持）
    "current_location": {"base_id": "01_HOME", "display_name": "月光荘（私の部屋）", "category": "REST"},
    ...
    "active_skill_name": "",
    "active_skill_effect": "",
    "last_dialogue": {"main": "", "sub": ""},
    "met_main": True,
    "met_sub": False,
    "both_first_turn": True,
    "pending_edits": {},
    "edit_mode": None,
...
# ★追加: 配布用カード（KARESHI_CARDS）の保存場所定義
def get_card_dir() -> str:
    # EXEの隣の "UserData/CARDS" を使用
    path = os.path.join(EXTERNAL_DIR, "UserData", "CARDS")
...
SAVE_KEYS = [
  ...
  "world_detail",
  "create_target","main_heroine","sub_heroine",
  "relationship_data","intro_text","start_choice",
  "game_initialized","day_count","time_of_day","location_text",
  "scene_party","current_route","active_speaker",
  "met_main","met_sub","route_choice_pending","both_first_turn",
...
def save_game_state(manual_save=False):
    ...
    if route == "main":
        ...
        h = st.session_state.get("chat_sub_heroine")
        if h: h_name = getattr(h, "name", "サブ")
    else:
        h_name = "二人"
    ...
    payload = {
        ...
        "heroine_sub": heroine_to_save(st.session_state.get("chat_sub_heroine")),
    }
...
def load_game_state(path: str):
    ...
    try:
        ...
        if isinstance(hs, dict):
            ...
            st.session_state.chat_sub_heroine = s_obj
        else:
            st.session_state.chat_sub_heroine = None
...
if "chat_heroine" not in st.session_state:
    st.session_state.chat_heroine = None
if "chat_sub_heroine" not in st.session_state:
    st.session_state.chat_sub_heroine = None
...
if "both_debug" not in st.session_state:
    st.session_state.both_debug = ""
if "both_first_turn" not in st.session_state:
    st.session_state.both_first_turn = True
    st.session_state.both_debug = ""
...
if "last_dialogue" not in st.session_state:
    st.session_state.last_dialogue = {"main": "", "sub": ""}
...
def compute_initial_bars(rd: dict, target: str, personality: str, job: str = ""):
    # target: "main" or "sub"
    if target == "sub":
    ...
    # ==================================================
    # ★ NEW: 職業・性格による「女好き（口説き）」補正
    # ==================================================
    # ガードを下げるキーワード（職業＆性格）
    flirty_keywords = [
        "ホスト", "バーテン", "美容師", "バンド", "モデル", "俳優", "スカウト", # 職業
        "女好き", "チャラ", "軟派", "フェミニスト", "遊び人", "人懐っこい", "小悪魔", "甘え上手" # 性格
    ]
    
    # 判定対象の文字列
    check_text = (job + personality).replace(" ", "")
    
    # キーワードが含まれていたらガードを大幅ダウン（＝いきなり口説きモード）
    is_flirty = any(k in check_text for k in flirty_keywords)
    
    # 補正値の計算
    guard_mod = 0
    if is_flirty:
        # 女好きキャラは、最初からガードが低い（40以下＝口説き解禁ライン）
        # ただし理性（reason）は少し残して「遊び」感を出す
        love += 20
        tokimeki += 30
        guard_mod = -40  # 劇的に下げる
        print(f"✨ Flirty Bonus Applied: {job} / {personality}")

    return love, tokimeki, reason, possession, guard_mod
...
"""
    ...
    if current_lang == "en":
        system_prompt = "You are an excellent scenario writer for romance ADV games. You must strictly follow the conditions (narration only, end just before dialogue). You MUST respond entirely in English."
    elif current_lang == "zh-CN":
        system_prompt = "你是一位优秀的恋爱ADV游戏剧本作家。你必须严格遵守条件（仅旁白，在对话前结束）。你必须完全用简体中文回复。"
    elif current_lang == "zh-TW":
        system_prompt = "你是一位優秀的戀愛ADV遊戲劇本作家。你必須嚴格遵守條件（僅旁白，在對話前結束）。你必須完全用繁體中文回覆。"
    else:
...
def get_active_heroine_and_key():
    route = st.session_state.get("current_route", "main")
    if route == "sub" and st.session_state.get("chat_sub_heroine") is not None:
        return st.session_state.chat_sub_heroine, "sub"
...
def get_active_heroine():
    route = st.session_state.get("current_route", "main")
    if route == "sub":
        return st.session_state.get("chat_sub_heroine", None), "sub"
...
def get_present_heroines():
    """
    Returns list of tuples: [("main", main_heroine_obj), ("sub", sub_heroine_obj)] filtered by scene_party.
    """
    party = st.session_state.get("scene_party", {"main": True, "sub": False})
    out = []
    if party.get("main") and st.session_state.get("chat_heroine") is not None:
        out.append(("main", st.session_state.chat_heroine))
    if party.get("sub") and st.session_state.get("chat_sub_heroine") is not None:
        out.append(("sub", st.session_state.chat_sub_heroine))
    return out

def check_is_both_day():
    party = st.session_state.get("scene_party", {"main": True, "sub": False})
    return bool(party.get("main")) and bool(party.get("sub")) and (st.session_state.get("chat_sub_heroine") is not None)
...
def handle_input(user_input, chat_ph=None):
    ...
    if IS_DEMO_MODE:
        import random
        # demo_man_002.png以降のファイルを探す
        demo_dir = os.path.join(BASE_DIR, "assets", "demo_heroine")
        demo_images = []
        if os.path.exists(demo_dir):
            for i in range(2, 100):  # demo_man_002.png から demo_man_099.png まで
                img_path = os.path.join(demo_dir, f"demo_man_{i:03d}.png")
    ...
    def _parse_and_update_location(resp_text):
        ...
        # 2. タグ削除
        cleaned_text = re.sub(r"<loc>.*?</loc>", "", resp_text, flags=re.DOTALL).strip()
        return cleaned_text
    ...
    try:
        ...
        if st.session_state.current_route == "sub":
            h = st.session_state.chat_sub_heroine
        ...
        # ==================================================
            ...
            # 3. ペナルティ判定（修正版：暴言・侮辱のみを弾く）
            # 日常会話（仕事、親友など）は削除。明らかに失礼な言葉のみをNGワードに設定。
            danger_keywords = [
                "奴隷", "下僕", "ペット", "ゴミ", "死ね", 
                "キモい", "ウザい", "消えろ", "ブス", "愛玩具"
            ]
            ...
            # ガード値に関係なく、暴言が含まれていれば即ペナルティ発動
            if has_danger:
                penalty_triggered = True
                # 好感度半減、ガードMAX
                h.love = int(getattr(h, "love", 0) * 0.5)
                h.guard = 100 
                st.toast(lang_mgr.get("text_0000", "不適切な発言を検知しました（激怒）"), icon="💢")
            ...
            if IS_DEMO_MODE:
                ...
                # 通常モード: ターン制限なし（体験版と同じく15ターン制限を適用しない）
                st.session_state.injection_prompt = ""
        ...
        # ▼ 通常時の許容ガイドライン（R15用）
             ...
             # キャラの職業と趣味を取得（なければ汎用的な言葉にする）
             c_job = getattr(h, "job", "仕事")
             c_hobby = getattr(h, "hobby", "野暮用")
             if not c_job: c_job = "仕事"
             if not c_hobby: c_hobby = "先約"

             permission_guide = f"""
            【❤️ 行動・優先順位の判定ガイド（Game Rule）】
            現在の「ガード値（Guard）」は、あなたの**「心の壁（他人行儀さ）」**を表しています。
            
            **現在のステータス:**
            - Tokimeki(好意): {current_tokimeki}
            - Guard(心の壁): {current_guard} (低いほど心を許している)

            **判定基準:**
            1. **Guard 40以上（他人 ＞ 恋心）**
               - まだあなたは、彼を「客」や「知人」として扱っています。礼儀正しく、しかし壁を作ってください。
               - **【重要】誘いの断り方:**
                 - **「親友」という言葉は絶対に使用禁止です。**（初対面で唐突すぎます）
                 - 代わりに、あなたの設定に基づいた理由を使ってください。
                 - **使用すべき理由:** 「{c_job}が忙しい」「{c_hobby}の時間だから」「少し疲れている」など。
               - しかし、**「なぜか彼を目で追ってしまう」「放っておけない」**という矛盾した行動（運命の引力）は描写してください。

            2. **Guard 40未満（恋心 ＞ 他人）**
               - あなたの心の壁は崩壊しました。彼があなたの世界の中心になります。
               - {c_job}や{c_hobby}よりも、彼と一緒にいる時間を優先したくなります。
               - ここで初めて、甘い言葉や独占欲、明確な求愛行動を解禁してください。
               - 「もう他人行儀はやめる」「お前が欲しい」という情熱をぶつけてください。
            """
             sys_prompt += "\n" + permission_guide
        ...
        # 描写ルール
        mn = getattr(h, "name", "カレ")
        NARRATION_RULES = """
        【重要：地の文（ナレーション）の執筆ルール】
        1. **視点の定義：「私」による観察描写**
           - 地の文の主語・視点は必ず「私（主人公）」にすること。
           - ただし、**私の「感情・思考」を書くことは禁止**する。
           - あくまで「私が見たもの」「私に聞こえたもの」という**事実の観察**に徹すること。
        ...
        target_h_name = getattr(h, "name", "カレ")
        ...
        # ▼▼▼ 恋人未満 10ターン強制終了ロジックは削除（体験版と同じく15ターン制限のみ適用） ▼▼▼
        # 体験版では15ターン制限（14回目でエンディング、15回目でHPリンク）が適用されるため、
        # 通常モードの10ターン制限は削除しました。
        ...
        with st.spinner(f"{getattr(h, 'name', 'カレ')} が考え中..."):
            for i in range(3):
                try:
        ...
        if decay_val > 0:
            ...
            st.toast(lang_mgr.get("text_0003", "会話の余韻で……彼のガードが少し柔らかくなった（-{decay_val}）"), icon="💭")
        ...
        # 例: "佐条 瑞希" -> "佐条瑞希"
        raw_h_name = getattr(h, "name", "カレ")
        safe_h_name = raw_h_name.replace(" ", "").replace("　", "")
        
        # 現在のルート（アイコン出し分け用）
        hkey = "main"
        if st.session_state.get("current_route") == "sub":
            hkey = "sub"
        ...
        if locals().get("apply_snowmelt", False):
            # AI更新後の最新値を取得
...
def game_start_dummy_if_needed():
    ...
    if isinstance(main_saved, dict):
        ...
        personality = ui.get("Personality", "")
        job_val = ui.get("Job", "") # 職業を取得
        rd = st.session_state.get("relationship_data", {}) or {}
        
        # job引数を追加し、戻り値のguard_modを受け取る
        lv, ls, rs, ps, g_mod = compute_initial_bars(rd, "main", personality, job_val)
        ...
        # ▼ 保存されたステータスがあればそれを優先、なければデフォルト(50)に補正を加える
        fs = main_saved.get("final_status", {}) or {}
        base_guard = fs.get("Guard", 50)
        h_data["Guard"] = clamp01(base_guard + g_mod) # 補正値を適用
    else:
        # フォールバック（ダミーではなく assets/male_main.json をロード）
        import json
        ...
        default_h_data = {
            ...
            "age": "32",
            "job": "秘書課長",
            "appearance": "黒髪、スーツ、眼鏡",
            "personality": "クール",
            "tone": "敬語",
            "first_line": "「……お待たせしました。」",
            "visual_tags": "1boy, suit, glasses, black hair",
            "love": 10, "tokimeki": 0, "reason": 90, "guard": 50
        }
        
        try:
            # 作成したJSONを読み込む
            json_path = os.path.join(BASE_DIR, "assets", "male_main.json")
            if os.path.exists(json_path):
                with open(json_path, "r", encoding="utf-8") as f:
                    src = json.load(f)
                
                # JSONデータをHeroineクラスの形式にマッピング
                default_h_data.update({
                    "name": src["basic_info"]["name"],
                    "age": src["basic_info"]["visual_age"].replace("歳",""),
                    "job": src["basic_info"]["job"],
                    "appearance": f"{src['visuals']['hair']} {src['visuals']['eyes']} {src['visuals']['fashion']}",
                    "personality": src["personality"],
                    "tone": src["basic_info"]["tone"],
                    "body_type": src["visuals"]["body_type"],     # 新設キーへ
                    "physical_trait": src["visuals"]["bust"],     # 胸板情報をここへ
                    "libido": "普通",  # テキストから推測または固定
                    "experience": "慣れ切っている",
                    "sensitivity": "鈍い",
                    # 秘密データ
                    "secret_fetish": src["secret_trait"]["content"],
                    "secret_fetish_desc": src["secret_trait"]["description"],
                    "secret_fetish_unlocked": src["secret_trait"]["is_revealed"]
                })
        except Exception as e:
            print(f"Fallback Load Error: {e}")

        # データをセット
        h_data = default_h_data

    st.session_state.chat_heroine = Heroine(h_data)

    # --- Sub Heroine Restore (if exists) ---
    sub_save_path = (st.session_state.get("sub_heroine") or {}).get("save_path", "")
    sub_saved = load_heroine_from_save(sub_save_path)

    if isinstance(sub_saved, dict):
        ui2 = sub_saved.get("user_input", {})
        ft2 = sub_saved.get("final_texts", {})
        sub_h_data = {
            "name": ui2.get("Name", "サブのカレ"),
            "age": ui2.get("Visual Age", "20"),
            "job": ui2.get("Job", "不明"),
            "appearance": ui2.get("Appearance", ""),
            "personality": ui2.get("Personality", ""),
            "hobby": ui2.get("Hobby", ""),
            "tone": ui2.get("Tone", ""),
            "backstory": ft2.get("main_profile", ""),
            "first_line": "...",
            "visual_tags": ft2.get("image_tags", ""),
            "location": "部屋",
            "bg_tag": "room",
            "bg_tag": "room",
            "image_path": (st.session_state.get("sub_heroine") or {}).get("image_path", ""),
            # ★追加: R15用ステータス読み込み
            "libido": (sub_saved.get("final_status", {}) or {}).get("Libido", "普通"),
            "experience": (sub_saved.get("final_status", {}) or {}).get("Experience", "普通"),
            "sensitivity": (sub_saved.get("final_status", {}) or {}).get("Sensitivity", "普通"),
            "secret_fetish_unlocked": (sub_saved.get("final_status", {}) or {}).get("secret_fetish_unlocked", False),
            
            # Hidden Traits (New)
            "breast_desc": ui2.get("breast_desc", "不明"),
            "vagina_desc": ui2.get("vagina_desc", "標準"),
            "vagina_note": ui2.get("vagina_note", ""),
            "secret_fetish": ui2.get("secret_fetish", "なし"),
            "secret_fetish_desc": ui2.get("secret_fetish_desc", ""),
            "secret_fetish_unlocked": (sub_saved.get("final_status", {}) or {}).get("secret_fetish_unlocked", False),
        }
        
        # Calculate Initial Stats (Sub)
        personality2 = ui2.get("Personality", "")
        job_val2 = ui2.get("Job", "") # 職業を取得
        
        # job引数を追加
        lv2, ls2, rs2, ps2, g_mod2 = compute_initial_bars(rd, "sub", personality2, job_val2)
        
        sub_h_data["love"] = lv2
        sub_h_data["tokimeki"] = ls2
        sub_h_data["reason"] = rs2
        sub_h_data["possession"] = ps2

        # ▼ 追加
        fs2 = sub_saved.get("final_status", {}) or {}
        base_guard2 = fs2.get("Guard", 50)
        sub_h_data["Guard"] = clamp01(base_guard2 + g_mod2) # 補正値を適用
        
        st.session_state.chat_sub_heroine = Heroine(sub_h_data)
    else:
        st.session_state.chat_sub_heroine = None
    ...
    if "### 🎬" in saved_intro:
        ...
        # 体験版: 場所情報を設定（雑居ビルの非常階段）
        from config import IS_DEMO_MODE
        if IS_DEMO_MODE:
            if "current_location" not in st.session_state or not st.session_state.current_location.get("display_name"):
                st.session_state.current_location = {
                    "base_id": "08_DUNGEON",
                    "display_name": "雑居ビルの非常階段",
                    "category": "DANGER"
                }
        # ※通常モードでは場所情報は作成フェーズですでに初期化されているため、ここでの解析はスキップします。
    ...
    if IS_DEMO_MODE and final_text:
        ...
        current_lang = st.session_state.get("language", "jp")
        if current_lang == "en":
            dialogue_pattern = r'Keitaro Kaburagi["「]([^"」]+)["」]'
        elif current_lang == "zh-CN":
            dialogue_pattern = r'鏑木圭太朗「([^」]+)」'
        elif current_lang == "zh-TW":
            dialogue_pattern = r'鏑木圭太朗「([^」]+)」'
        else:
            dialogue_pattern = r'鏑木圭太朗「([^」]+)」'
        ...
        if dialogue_match:
            ...
            # セリフ部分を導入文から削除（複数行に対応）
            if current_lang == "en":
                intro_narrative = re.sub(r'\n?Keitaro Kaburagi["「][^"」]+["」]', '', final_text)
            else:
                intro_narrative = re.sub(r'\n?鏑木圭太朗「[^」]+」', '', final_text)
    ...
    if sc == "main":
        st.session_state.scene_party = {"main": True, "sub": False}
        st.session_state.current_route = "main"
        st.session_state.active_speaker = "main"

    elif sc == "sub":
        st.session_state.scene_party = {"main": False, "sub": True}
        st.session_state.current_route = "sub"
        st.session_state.active_speaker = "sub"

    else:  # both
        st.session_state.scene_party = {"main": True, "sub": True}
        st.session_state.current_route = "main"
        st.session_state.active_speaker = "both"

    # Safety: check if sub exists
    if st.session_state.chat_sub_heroine is None:
        st.session_state.scene_party["sub"] = False
        if st.session_state.current_route == "sub":
             st.session_state.current_route = "main"
        if st.session_state.active_speaker == "sub":
             st.session_state.active_speaker = "main"
    
    # Init Image
    set_current_image_to_base(st.session_state.current_route)
    ...
    if "met_sub" not in st.session_state:
        st.session_state.met_sub = False

    if "route_choice_pending" not in st.session_state:
        st.session_state.route_choice_pending = False
...
def set_current_image_to_base(route: str):
    # route: "main" or "sub"
    # Determines which heroine's saved image to use based on the route
    if route == "sub":
        hero = st.session_state.get("sub_heroine") or {}
    else:
...
@st.dialog(lang_mgr.get("text_0005", "翌朝、今日は誰と過ごす？"))
def route_choice_dialog():
    main_h = st.session_state.get("chat_heroine")
    sub_h  = st.session_state.get("chat_sub_heroine")
    main_name = getattr(main_h, "name", "メイン")
    sub_name  = getattr(sub_h, "name", "サブ")
    
    # Current route (Yesterday's partner)
    # This value has not been updated yet, so it represents the "Previous" route.
    prev_route = st.session_state.get("current_route", "main")

    st.caption(lang_mgr.get("text_0006", "（選択した相手が会話と画像生成の対象になる）"))

    # Helper for penalty logic
    def apply_switch_penalty(target_h, h_name):
        if target_h:
            # 1. Love/Tokimeki/Possession -> Half (*0.5)
            target_h.love = int(getattr(target_h, "love", 0) * 0.5)
            target_h.tokimeki = int(getattr(target_h, "tokimeki", 0) * 0.5)
            target_h.possession = int(getattr(target_h, "possession", 0) * 0.5)
            
            # 2. Reason/Guard -> 1.2x (*1.2)
            cur_reason = int(getattr(target_h, "reason", 0))
            target_h.reason = min(100, int(cur_reason * 1.2))
            
            cur_guard = int(getattr(target_h, "guard", 0))
            target_h.guard = min(100, int(cur_guard * 1.2))
            
            print(f"🔄 Switch Penalty Applied to {h_name}")
            st.toast(lang_mgr.get("text_0007", "心変わりにより、{h_name} の心が離れました..."), icon="💔")

    c1, c2, c3 = st.columns(3)
    
    # --- Button 1: Pick MAIN ---
    if c1.button(main_name, width="stretch", key="dlg_pick_main"):
        # Logic: If coming from SUB (Single), apply penalty to SUB (Left behind)
        if prev_route == "sub":
             apply_switch_penalty(sub_h, sub_name)
    ...
    # --- Button 2: Pick SUB ---
    if c2.button(sub_name, width="stretch", key="dlg_pick_sub"):
        # Logic: If coming from MAIN (Single), apply penalty to MAIN (Left behind)
        if prev_route == "main":
             apply_switch_penalty(main_h, main_name)

        st.session_state.current_route = "sub"
        st.session_state.scene_party = {"main": False, "sub": True}
        st.session_state.active_speaker = "sub"
        st.session_state.route_choice_pending = False
        set_current_image_to_base("sub")
        st.rerun()
        
    # --- Button 3: Pick BOTH ---
    if c3.button("両方", width="stretch", key="dlg_pick_both"):
        # BOTH does not trigger penalty
        st.session_state.current_route = "main"
        st.session_state.scene_party = {"main": True, "sub": True}
        st.session_state.active_speaker = "both"
        st.session_state.route_choice_pending = False
...
@st.dialog("📖 カレの秘密の日記（Memory）")
def show_memory_dialog():
    st.caption("※カレが夜、こっそり書き留めている日記のようです……")
    st.divider()

    # Determine target (Main or Sub) based on current route or default to Main
    target = st.session_state.get("chat_heroine")
    current_route = st.session_state.get("current_route", "main")
    if current_route == "sub" and st.session_state.get("chat_sub_heroine"):
         target = st.session_state.get("chat_sub_heroine")
    
    if not target:
        st.error(lang_mgr.get("text_0032", "カレのデータが見つかりません。"))
        if st.button(lang_mgr.get("text_0011", lang_mgr.get("text_0016", lang_mgr.get("text_0019", lang_mgr.get("text_0025", lang_mgr.get("text_0028", lang_mgr.get("text_0033", lang_mgr.get("text_0035", "閉じる")))))))): st.rerun()
        return
...
""",unsafe_allow_html=True)
    ...
    sub_h = st.session_state.get("chat_sub_heroine", None)

    party = st.session_state.get("scene_party", {"main": True, "sub": False})
    route = st.session_state.get("current_route", "main")

    has_main = bool(party.get("main")) and (main_h is not None)
    has_sub  = bool(party.get("sub")) and (sub_h is not None)
    
    is_both = has_main and has_sub

    # Auto-correction for single days
    if not is_both:
        if route == "sub" and not has_sub:
            route = "main"
            st.session_state.current_route = "main"
        if route == "main" and not has_main:
            route = "sub"
            st.session_state.current_route = "sub"
    ...
    with col_left:
        # 体験版バッジを左カラム上部に表示（クリック可能なリンク）
        from config import IS_DEMO_MODE, DEMO_HP_URL
        if IS_DEMO_MODE:
            demo_x_url = DEMO_HP_URL
            st.markdown(f"""
            <div style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); padding: 8px; border-radius: 8px; text-align: center; margin-bottom: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); cursor: pointer; transition: transform 0.2s ease;" onclick="window.open('{demo_x_url}', '_blank')" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
                <a href="{demo_x_url}" target="_blank" style="color: white; font-weight: bold; font-size: 14px; text-decoration: none; display: block;">🎮 体験版（公式Xはこちら）</a>
            </div>
            """, unsafe_allow_html=True)

        # Card Rendering
        if is_both:
            components.render_character_card(main_h, components.MAIN_COLORS, is_active=(route == "main"), show_debug=False)
            st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
            components.render_character_card(sub_h,  components.SUB_COLORS,  is_active=(route == "sub"), show_debug=False)
        else:
            if route == "main" and has_main:
                components.render_character_card(main_h, components.MAIN_COLORS, is_active=True, show_debug=False)
            elif route == "sub" and has_sub:
                components.render_character_card(sub_h, components.SUB_COLORS, is_active=True, show_debug=False)
        ...
        with gs1:
            save_label = lang_mgr.get("text_0167", "SAVE")
            if IS_DEMO_MODE:
                disabled_text = lang_mgr.get("text_0165", f"💾 {save_label} (体験版では利用できません)")
                help_text = lang_mgr.get("text_0166", "体験版ではセーブ機能は利用できません")
        ...
        with gs2:
            load_label = lang_mgr.get("text_0170", "LOAD")
            if IS_DEMO_MODE:
                disabled_text = lang_mgr.get("text_0168", f"📖 {load_label} (体験版では利用できません)")
                help_text = lang_mgr.get("text_0169", "体験版ではロード機能は利用できません")
        ...
        if st.button(lang_mgr.get("text_0038", "📖 日記を読む"), width="stretch", help=lang_mgr.get("text_0039", "カレ視点の思い出を振り返ります")):
            show_memory_dialog()

        # ★追加: エクスポートボタン (日記ボタンの下に配置)
        if IS_DEMO_MODE:
            disabled_text = lang_mgr.get("text_0173", "💌 共有 (Export) (体験版では利用できません)")
            help_text = lang_mgr.get("text_0174", "体験版ではエクスポート機能は利用できません")
            st.button(disabled_text, width="stretch", key="btn_export_card", disabled=True, help=help_text)
        else:
            if st.button(lang_mgr.get("text_0175", "💌 共有 (Export)"), width="stretch", key="btn_export_card", help=lang_mgr.get("text_0176", "カレのデータを『カード』として書き出します（会話ログは含まれません）")):
                import json
                import re
                import base64
                from datetime import datetime
            
            # 保存データ作成（ログ・日記を除外したクリーンデータ）
            img_b64 = ""
            if st.session_state.get("current_image_bytes"):
                try:
                    img_b64 = base64.b64encode(st.session_state.current_image_bytes).decode('utf-8')
                except: pass

            export_data = {
                "format": "kareshi_card_v1",
                "user_input": dict(st.session_state.get("user_input", {})),
                "final_texts": dict(st.session_state.get("final_texts", {})),
                "final_status": dict(st.session_state.get("final_status", {})),
                "generated_theme": st.session_state.get("generated_theme", ""),
                "image_b64": img_b64,
                "created_at": datetime.now().strftime("%Y/%m/%d %H:%M"),
                "note": "Shared Card Data"
            }
            
            try:
                save_dir = get_card_dir()
                name = st.session_state.get("user_input", {}).get("Name", "NoName")
    ...
    with col_center:
        ...
        if st.session_state.chat_heroine:
            # --- 2.5 Call Other Heroine (Mid-Game Join) ---
            # Check if we can call the other heroine
            party = st.session_state.get("scene_party", {"main": True, "sub": False})
            is_both = party.get("main") and party.get("sub")
            
            call_candidate = None
            call_target_key = None
            
            # Logic: If NOT both, find who is missing
            if not is_both:
                if party.get("main") and not party.get("sub"):
                    # Current is Main, check if Sub exists
                    sub_h_obj = st.session_state.get("chat_sub_heroine")
                    if sub_h_obj:
                        call_candidate = sub_h_obj.name
                        call_target_key = "sub"
                elif party.get("sub") and not party.get("main"):
                    # Current is Sub, check if Main exists
                    main_h_obj = st.session_state.get("chat_heroine")
                    if main_h_obj:
                        call_candidate = main_h_obj.name
                        call_target_key = "main"
            ...
            with ac1:
                if st.button(lang_mgr.get("text_0041", "💬 無難"), help=lang_mgr.get("text_0042", "優しく微笑む・安心感・包容力"), width="stretch"):
                    chosen_tone = "safe"
            with ac2:
                if st.button(lang_mgr.get("text_0043", "😈 攻め"), help=lang_mgr.get("text_0044", "ちょっと強引に距離を詰める"), width="stretch"):
                    chosen_tone = "bold"
            with ac3:
            ...
            with sc2:
                if st.session_state.get("is_skill_active", False):
                    ...
                    if st.button(lang_mgr.get("text_0048", "解除"), type="primary", width="stretch", key="btn_skill_release"):
                        ...
                        targets = []
                        spk = st.session_state.get("active_speaker", "main")
                        if spk == "both":
                            targets = [st.session_state.chat_heroine, st.session_state.chat_sub_heroine]
                        elif spk == "sub":
                            targets = [st.session_state.chat_sub_heroine]
                        else:
                        ...
                        if "アクシデント" in skill_name:
                            ...
                            st.session_state.chat_history.append({"role": "model", "parts": [release_text], "speaker_name": "System"})
                    ...
                    if st.button(lang_mgr.get("text_0051", "発動"), width="stretch", key="btn_skill_activate"):
                        ...
                        if st.session_state.active_skill_name == "✨ 自由記述":
                             ...
                             current_lang = st.session_state.get("language", "jp")
                             if current_lang == "en":
                                 start_msg = f"(The common sense of the world has been altered: {custom_text})"
                                 during_msg = f"【World Law (Highest Priority・Absolute)】{custom_text}\n- This law is the \"natural common sense\" of this world, and the characters do not question it at all.\n- The characters are not passive, but naturally take proactive actions according to common sense.\n- In the next response, you must describe at least one result of the law as an \"action\" (no words only)."
                                 end_msg = "(The common sense alteration has been released, returning to the original values.)"
                                 narration_text = f"{lang_mgr.get('text_0198', '（スキル発動：自由記述）')}\n('{custom_text}' {lang_mgr.get('text_0199', 'がこの世界の常識になった。')}\n{lang_mgr.get('text_0200', '彼は最初からそうだったかのように受け入れている。')})"
                             elif current_lang == "zh-CN":
                                 start_msg = f"（世界的常识被改变：{custom_text}）"
                                 during_msg = f"【世界法则（最高优先级・绝对）】{custom_text}\n- 这个法则是这个世界的\"理所当然的常识\"，角色们不会产生任何疑问。\n- 角色不是被动的，而是根据常识自然地采取主动行动。\n- 在下一个回复中，必须至少将一个法则的结果作为\"行动\"来描述（禁止只有话语）。"
                                 end_msg = "（常识改变已解除，恢复到原来的价值观）"
                                 narration_text = f"{lang_mgr.get('text_0198', '（スキル発動：自由記述）')}\n（'{custom_text}'{lang_mgr.get('text_0199', 'がこの世界の常識になった。')}\n{lang_mgr.get('text_0200', '彼は最初からそうだったかのように受け入れている。')}）"
                             elif current_lang == "zh-TW":
                                 start_msg = f"（世界的常識被改變：{custom_text}）"
                                 during_msg = f"【世界法則（最高優先級・絕對）】{custom_text}\n- 這個法則是這個世界的\"理所當然的常識\"，角色們不會產生任何疑問。\n- 角色不是被動的，而是根據常識自然地採取主動行動。\n- 在下一個回覆中，必須至少將一個法則的結果作為\"行動\"來描述（禁止只有話語）。"
                                 end_msg = "（常識改變已解除，恢復到原來的價值觀）"
                                 narration_text = f"{lang_mgr.get('text_0198', '（スキル発動：自由記述）')}\n（'{custom_text}'{lang_mgr.get('text_0199', 'がこの世界の常識になった。')}\n{lang_mgr.get('text_0200', '彼は最初からそうだったかのように受け入れている。')}）"
                             else:
                                 start_msg = f"（世界の常識が改変される：{custom_text}）"
                                 during_msg = f"【世界法則（最優先・絶対）】{custom_text}\n- この法則はこの世界の\"当然の常識\"であり、登場人物は一切疑問に思わない。\n- 登場人物は受け身ではなく、常識に沿って自然に能動行動を取る。\n- 次のレスポンスでは、必ず\"行動\"として法則の結果を最低1つ描写する（口だけ禁止）。"
                                 end_msg = "（常識改変が解除され、元の価値観に戻る）"
                                 narration_text = f"{lang_mgr.get('text_0198', '（スキル発動：自由記述）')}\n（『{custom_text}』{lang_mgr.get('text_0199', 'がこの世界の常識になった。')}\n{lang_mgr.get('text_0200', '彼は最初からそうだったかのように受け入れている。')}）"
                             ...
                             if sel_key in SKILL_DEFINITIONS:
                                 current_skill_data = SKILL_DEFINITIONS[sel_key].copy()
                                 
                                 # アクシデントの実の場合は多言語対応のテキストに置き換え
                                 if "アクシデント" in sel_key:
                                     current_skill_data["start"] = lang_mgr.get("text_0192", current_skill_data["start"])
                                     # duringテキストも多言語対応（LLMへの指示）
                                     if current_lang == "en":
                                         current_skill_data["during"] = "【Accident Fruit (Active)】\n・Due to magical force, his body and the player's body are stuck together and cannot separate.\n・The more they try to untangle, the more entangled they become.\n・Each other's body temperature and heartbeat are directly transmitted, and he feels extreme tension and embarrassment.\n・**Important: Until released, maintain the state where his and the player's bodies are stuck together.**\n・Continue to describe the state where their bodies are touching or at a very close distance.\n・Even in dialogue and action descriptions, maintain this stuck state."
                                     elif current_lang == "zh-CN":
                                         current_skill_data["during"] = "【意外之果（生效中）】\n・由于魔法力量，他和玩家的身体紧贴在一起无法分离。\n・越是想分开，就越会纠缠在一起。\n・彼此的体温和心跳直接传递，他感到极度紧张和害羞。\n・**重要：在解除之前，保持他和玩家的身体紧贴在一起的状态。**\n・继续描述他们的身体接触或处于非常近距离的状态。\n・即使在对话和行动描述中，也要保持这种紧贴状态。"
                                     elif current_lang == "zh-TW":
                                         current_skill_data["during"] = "【意外之果（生效中）】\n・由於魔法力量，他和玩家的身體緊貼在一起無法分離。\n・越是想分開，就越會糾纏在一起。\n・彼此的體溫和心跳直接傳遞，他感到極度緊張和害羞。\n・**重要：在解除之前，保持他和玩家的身體緊貼在一起的狀態。**\n・繼續描述他們的身體接觸或處於非常近距離的狀態。\n・即使在對話和行動描述中，也要保持這種緊貼狀態。"
                                 ...
                                 if current_lang == "en":
                                     ...
                                     fallback_start = "(発動)"
                                 current_skill_data = {"start": fallback_start, "during": "", "end": ""}
                                 narration_text = fallback_start
                        ...
                        if skill_bonus:
                            targets = []
                            spk = st.session_state.get("active_speaker", "main")
                            if spk == "both": targets = [st.session_state.chat_heroine, st.session_state.chat_sub_heroine]
                            elif spk == "sub": targets = [st.session_state.chat_sub_heroine]
                            else: targets = [st.session_state.chat_heroine]
                            
                            feedback_list = []
                            
                            # Prepare toast message parts (多言語対応)
                            if skill_bonus.get("chastity", 0) != 0:
                                guard_label = lang_mgr.get("text_0189", "ガード")
                                feedback_list.append(f"{guard_label}{skill_bonus['chastity']:+}")
                            if skill_bonus.get("lust", 0) != 0:
                                lust_label = lang_mgr.get("text_0186", "興奮度")
                                feedback_list.append(f"{lust_label}{skill_bonus['lust']:+}")
                            if skill_bonus.get("love", 0) != 0:
                                love_label = lang_mgr.get("text_0185", "好感度")
                                feedback_list.append(f"{love_label}{skill_bonus['love']:+}")
                            ...
                            if feedback_list:
                                ...
                                skill_name = st.session_state.active_skill_name
                                toast_template = lang_mgr.get("text_0056", "{skill_name}発動！ {msg}")
                                toast_msg = toast_template.format(skill_name=skill_name, msg=msg)
                        ...
                        with st.spinner(lang_mgr.get("text_0057", "世界改変中...彼の様子が変化しています...")):
                             # 現在のヒロイン取得
                             h_obj = st.session_state.chat_heroine
                             if st.session_state.current_route == "sub": h_obj = st.session_state.chat_sub_heroine
                             ...
                             【状況】
                             たった今、世界改変スキルが発動しました。
                             直前のSystemメッセージにある変更内容に従い、カレの態度や表情が変化する様子を描写してください。
                             ...
                             if res:
                                 ...
                                 import re
                                 clean_res = re.sub(r"<loc>.*?</loc>", "", res, flags=re.DOTALL).strip()
                                 clean_res = re.sub(r"\{base_id:.*?\}", "", clean_res, flags=re.DOTALL).strip()
                        ...
                        with st.spinner(lang_mgr.get("text_0060", "一日の思い出を日記に記しています...")):
                            ...
                            for h_mem in targets:
                                if h_mem:
                                    ...
                                    mem_prompt = f"""
以下の会話ログから、カレ「{h_mem.name}」が夜にこっそり書いた【今日の日記】を作成してください。
...
【執筆ガイド】
1. **一人称視点:** 「〜しました」という報告ではなく、「〜してドキドキした」「〜が嬉しかった」という**カレの独白・本音**形式にしてください。
2. **感情と五感:** 「手の温もり」「夕日の眩しさ」「彼女（プレイヤー）の匂い」など、感覚的な記憶を重視してください。
3. **具体的なエピソード:** 「デートした」とまとめるのではなく、「ネクタイを直してくれた優しさにときめいた」のように具体的に書いてください。
"""
                                    # 履歴をテキスト化
                                    ...
                                    try:
                                        ...
                                        title_prompt = f"""
あなたはコピーライターです。
カレ「{h_mem.name}」の日記と、現在の関係ランク「{current_status}」を元に、
...
"""
                    ...
                    if spk == "main":
                        ...
                        if st.session_state.get("met_sub", False):
                            add_bond(sub_h, base_inc + bonus_inc)
                    else:  # both
                        if st.session_state.get("met_main", True):
                            add_bond(main_h, base_inc + bonus_inc)
                        if st.session_state.get("met_sub", False):
                    ...
                    # --- 3. Stat Decay & Recovery (All Heroines) ---
                    for key in ["chat_heroine", "chat_sub_heroine"]:
                        h_obj = st.session_state.get(key)
                        if not h_obj:
                            continue
                        # Natural Decay
                    ...
                    if st.session_state.get("active_speaker") == "sub":
                        target_h = st.session_state.get("chat_sub_heroine")
                        ...
                        if random.random() < PEACEFUL_CHANCE:
                            ...
                            try:
                                ...
                                if os.path.exists(events_path):
                                    ...
                                    st.session_state.last_event_day = next_day
                                    st.toast(f"イベント発生！「{selected_event['level']}」", icon="⚡")
                                    ...
                                    【現在の状況】
                                    カレ: {target_h.name} (好感度: {current_love})
                                    ...
                                    2. 「具体的な出来事」と「カレの反応（セリフ・LINE）」を描写してください。
            ...
            if st.session_state.active_skill_name == "✨ 自由記述":
                ...
                custom_effect = st.text_input(
                    ...
                    value=current_val,
                    placeholder="例: 常識改変！裸が当たり前の世界にチェンジ！",
    ...
    with col_right:
        ...
        with c_gen:
            ...
            if IS_DEMO_MODE:
                disabled_text = lang_mgr.get("text_0177", "🖼️ シーン画像を生成 (体験版では利用できません)")
                help_text = lang_mgr.get("text_0178", "体験版では画像生成機能は利用できません")
                ...
                if st.button(lang_mgr.get("text_0068", "🖼️ シーン画像を生成"), key="btn_gen_scene_right", width="stretch"):
                 try:
                    ...
                    if not target_h:
                        st.error(lang_mgr.get("text_0069", "カレのデータがありません"))
                    else:
                        # DNA（髪型などの固定情報）
                        dna_tags = getattr(target_h, "visual_tags", "")
                        if not dna_tags: dna_tags = "1boy, handsome face"
                        ...
                        2. **服装（原則デフォルト維持）:**
                           - **基本ルール:** 何も指定がなければ、【DNA】に含まれる服装タグ（例: suit, casual, uniform）をそのまま使ってください。
        ...
        # --- Context for UI ---
        party = st.session_state.get("scene_party", {"main": True, "sub": False})
        has_main = bool(party.get("main")) and (st.session_state.get("chat_heroine") is not None)
        has_sub  = bool(party.get("sub")) and (st.session_state.get("chat_sub_heroine") is not None)
        route = st.session_state.get("current_route", "main")
        active_h_exists = (route == "main" and has_main) or (route == "sub" and has_sub)
    ...
    # --- Route Choice Modal ---
    if st.session_state.get("route_choice_pending", False):
         route_choice_dialog()
    ...
    if current_len > last_len:
        party = st.session_state.get("scene_party", {"main": True, "sub": False})
        is_both_active = (party.get("main") and party.get("sub"))
        
        # Userの発言位置を狙うオフセット
        scroll_offset = 4 if is_both_active else 2
...
def render_age_gate():
    ...
    with col_center:
        ...
        if current_key:
            ...
            st.info("👇 Setup Google Gemini API Key", icon="⚙️")
        ...
        # --- Warning & Button ---
        if IS_R18_APP:
            st.markdown('<div class="gate-warning">⚠️ <b>WARNING: R-18</b><br>18歳未満の方はプレイできません。<br>(Contains Adult Content)</div>', unsafe_allow_html=True)
            agree_label = "はい、私は18歳以上です (Start Game)"
        else:
            st.markdown('<div style="height:20px"></div>', unsafe_allow_html=True)
            agree_label = "ゲームを始める (Start Game)"

        # Start Button (Primary)
        if st.button(agree_label, type="primary", width="stretch"):
            if not input_key:
                st.error(lang_mgr.get("text_0076", lang_mgr.get("text_0107", "APIキーを入力してください")))
            else:
                # 設定保存
                ...
                try:
                    ...
                    st.error(lang_mgr.get("text_0077", lang_mgr.get("text_0108", "初期化エラー: {e}")))
...
def render_age_gate():
    ...
    with col_center:
        ...
        # 現在の言語設定でデータを読み込み
        current_lang = st.session_state.get("language", "jp")
        lang_mgr.load_data(current_lang, "female_target")
        ...
        if st.button(agree_label, type="primary", width="stretch", key="gate_start_btn"):
            if not input_key:
                ...
                if "language" in st.session_state:
                    lang_mgr.load_data(st.session_state.language, "female_target")
        ...
        # 見つからない場合は最初のキーを使用
        if current_lang_key is None:
            current_lang_key = list(lang_options.keys())[0]
        
        # インデックスを計算
        lang_keys_list = list(lang_options.keys())
        try:
            default_index = lang_keys_list.index(current_lang_key)
        except ValueError:
            default_index = 0
        
        selected_lang_key = st.selectbox(
            "",
            options=lang_keys_list,
            index=default_index,
            label_visibility="collapsed",
            key="gate_language_select"
        )
        
        # 選択された言語をセッションステートに保存
        selected_lang = lang_options[selected_lang_key]
...
def render_title_screen():
    ...
    with st.container():
        ...
        # カラム比率: [LOAD(1)] [IMPORT(1)] [START(2)]
        c_load, c_import, c_start = st.columns([1, 1, 2], gap="small")

        with c_load:
            # 📂 LOAD (既存)
            if st.button("📂 LOAD", key="btn_title_load", width="stretch"):
                load_menu_dialog() 

        with c_import:
            # 💌 IMPORT (新規)
            if st.button("💌 IMPORT", key="btn_title_import", width="stretch", help="『カレシカード』(json)を読み込んで、新しい恋を始めます"):
                import_card_dialog()

        with c_start:
            # 🚀 GAME START (既存)
            if st.button("GAME START", key="btn_title_start", width="stretch"):
                # Save Logic
...
def render_create_phase():
    ...
    if "final_texts" not in st.session_state:
        ...
        if IS_DEMO_MODE:
            # 体験版モード: 固定キャラクター「鏑木圭太朗」を読み込む（言語対応）
            # 注意: 女性向け版では男性キャラクター用のデータを使用
            current_lang = st.session_state.get("language", "jp")
            if current_lang == "en":
                demo_heroine_path = os.path.join(BASE_DIR, "assets", "demo_heroine", "kaburagi_keitaro_en.json")
            elif current_lang == "zh-CN":
                demo_heroine_path = os.path.join(BASE_DIR, "assets", "demo_heroine", "kaburagi_keitaro_zh-CN.json")
            elif current_lang == "zh-TW":
                demo_heroine_path = os.path.join(BASE_DIR, "assets", "demo_heroine", "kaburagi_keitaro_zh-TW.json")
            else:
                demo_heroine_path = os.path.join(BASE_DIR, "assets", "demo_heroine", "kaburagi_keitaro.json")
            ...
            try:
                ...
                # 画像を読み込む（最初の立ち絵はdemo_man_001.png）
                demo_image_path = os.path.join(BASE_DIR, "assets", "demo_heroine", "demo_man_001.png")
    ...
    if "final_texts" in st.session_state:
        ...
        with c_head_L:
            st.markdown(f"## 🌸 {st.session_state.user_input.get('Name')} プロフィールカード")
        with c_head_R:
        ...
        with col_txt:
            ...
            # 修正: タイトルと中身を変更
            with st.expander("💗 カレのヒミツ (クリックで見る)", expanded=False):
        ...
        if st.button("❤️ このカレと恋を始める (GAME START)", type="primary", width="stretch", key="btn_game_start_random"):
            ...
            if IS_DEMO_MODE:
                ...
                if current_lang == "en":
                    ...
                    default_intro = "### 🎬 Encounter\n\nI met a boy on the emergency staircase."
                    location_name = "Emergency Staircase"
                elif current_lang == "zh-CN":
                ...
                with st.spinner(lang_mgr.get("text_0127", "シチュエーションを二人に合わせて書き直しています...")): 
                    ...
                    try:
                        ...
                        current_lang = st.session_state.get("language", "jp")
                        if current_lang == "en":
                            rewrite_sys_prompt = "You are an excellent novelist. You MUST respond entirely in English."
                        elif current_lang == "zh-CN":
                            rewrite_sys_prompt = "你是一位优秀的小说家。你必须完全用简体中文回复。"
                        elif current_lang == "zh-TW":
                            rewrite_sys_prompt = "你是一位優秀的小說家。你必須完全用繁體中文回覆。"
                        else:
                            rewrite_sys_prompt = "あなたは優秀な小説家です。"
                        rewritten = client.generate_response(hist, rewrite_sys_prompt)
            ...
            # ★ NEW: プロローグ（主人公のモノローグ）を履歴の最初に追加
            player_name = st.session_state.get("user_name", "私")
            prologue_text = f"（私の名前は{player_name}。どこにでもいる平凡な女性だ。）\\n（住んでいるのは、東京都杉並区阿佐ヶ谷にある木造アパート『月光荘』。）\\n（中央線の音が遠くに聞こえるこの街で……私の運命を変える出会いが、すぐそこまで迫っていた。）"
            
            # 初期履歴をモノローグで上書き + 導入テキスト追加
            st.session_state.chat_history = [
                {"role": "model", "parts": [prologue_text], "speaker_name": "System"},
                {"role": "model", "parts": [st.session_state.intro_text], "speaker_name": "System"}
            ]

            # お祝いエフェクト
            st.balloons()
            st.toast(lang_mgr.get("text_0128", "カレの生成完了！物語が始まります。"), icon="🎉")
...
def render_relationship_phase():
    ...
    def build_intro_prompt(rd: dict, main_profile: str, sub_profile: str, mode: str, player_name: str, player_age: str, main_name: str, sub_name: str) -> str:
        ...
        - **一人称視点（{my_pronoun}視点）で書くこと**
        - 主語は「{my_pronoun}」。
        - {my_pronoun}の五感と感情（ときめき、動揺、決意など）を交えて描写せよ。
        - カレの心理は断定せず、{my_pronoun}から見た様子として書くこと。
        """
...
""".strip()
    ...
    # 0. Define main/sub/has_sub safely at start
    main = st.session_state.get("main_heroine")
    sub = st.session_state.get("sub_heroine")
    has_main = isinstance(main, dict)
    has_sub = isinstance(sub, dict)
    ...
    # 5) Sub Heroine Data (Load from Save)
    sub_save_path = (sub or {}).get("save_path", "")
    sub_saved = load_heroine_from_save(sub_save_path)
    ...
    with col_mid:
        ...
        with col_p2:
            ...
            rd["narrative_style"] = "一人称（私視点）"

        st.markdown("<hr>", unsafe_allow_html=True)

        # --- メインのカレ ---
        st.markdown(lang_mgr.get("text_0133", "### メインのカレとの関係"))
        ...
        # --- サブのカレ ---
        if has_sub:
            st.markdown(lang_mgr.get("text_0136", "### サブのカレとの関係"))
        ...
        if st.button(lang_mgr.get("text_0142", "メイン彼との導入生成"), width="stretch", key="btn_gen_main"):
            ...
            main_name = (main_saved or {}).get("user_input", {}).get("Name", "メインのカレ")
            sub_name = (sub_saved or {}).get("user_input", {}).get("Name", "サブのカレ")
            ...
            with st.spinner(lang_mgr.get("text_0143", "メイン導入を生成中...")): 
                ...
                current_lang = st.session_state.get("language", "jp")
                if current_lang == "en":
                    sys_prompt = "You are an excellent scenario writer. You MUST respond entirely in English."
                elif current_lang == "zh-CN":
                    sys_prompt = "你是一位优秀的剧本作家。你必须完全用简体中文回复。"
                elif current_lang == "zh-TW":
                    sys_prompt = "你是一位優秀的劇本作家。你必須完全用繁體中文回覆。"
                else:
        ...
        if st.button(lang_mgr.get("text_0150", "これでゲームスタート"), type="primary", width="stretch"):
            ...
            if st.session_state.get("sub_heroine"):
                apply_complex_correction(st.session_state.get("sub_heroine"), rd.get("sub_relation_choice"), rd.get("sub_relation_free"))
    ...
    with col_right:
        if isinstance(sub_saved, dict):
            ...
            ft = sub_saved.get("final_texts", {})
            img_p = ""
            if sub_save_path:
                cand = os.path.splitext(sub_save_path)[0] + ".png"
                if os.path.exists(cand):
                    img_p = cand
    ...
    with c1:
        if st.button(lang_mgr.get("text_0152", "↩ メインのカレへ戻る"), width="stretch"):
            st.session_state.create_target = "main"
            st.session_state.main_heroine = None
            st.session_state.sub_heroine = None
...
# ★追加: カレシカード読み込みダイアログ
@st.dialog("💌 カレを招待する (Import)")
def import_card_dialog():
    st.caption("UserData/CARDS フォルダにあるファイルを読み込みます。")
    st.info("※読み込むと『初対面』の状態から物語が始まります。")
    ...
    if not files:
        st.warning("カードが見つかりません (UserData/CARDS)")
        if st.button("閉じる"): st.rerun()
        return

    sel_file = st.selectbox("カードを選択", files)
    
    if st.button("❤️ このカレと恋を始める", type="primary", width="stretch"):
        try:
            ...
            # 1. プレイヤー情報をタイトル画面の入力値から確定させる
            # (タイトル画面のウィジェットキー 'title_name', 'title_age' を参照)
            ...
            # 2. ゲーム開始フラグを立てる（これがないとタイトルに戻される）
            st.session_state.protagonist_set = True
            ...
            # NTRでなければ導入もリセット
            is_ntr = data.get("is_ntr_data", False)
            if not is_ntr:
                st.session_state.intro_text = ""
                if "pending_ntr_data" in st.session_state:
                    del st.session_state.pending_ntr_data
...
if __name__ == "__main__":
    if getattr(sys, 'frozen', False):
        ...
        # ★ここが安全装置！
        # すでにStreamlitサーバーの中から呼ばれている場合は、サーバー起動をスキップしてゲームを開始する
        if os.environ.get("STREAMLIT_RUNNING_IN_FROZEN"):
            main()
        else:
            # 初回起動時（ユーザーがダブルクリックした時）
            # 「今からサーバーの中で動くよ！」という旗（フラグ）を立てる
            os.environ["STREAMLIT_RUNNING_IN_FROZEN"] = "true"
            
            from streamlit.web import cli as stcli
            target_script = os.path.join(sys._MEIPASS, "main.py")
