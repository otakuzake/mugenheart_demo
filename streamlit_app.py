...
if "gemini_client" not in st.session_state:
    if st.session_state.get("gemini_api_key"):
        # Web体験版: モデル固定
        st.session_state.gemini_client = GeminiClient(st.session_state.gemini_api_key, model_name=FIXED_MODEL)
    else:
        st.session_state.gemini_client = None
# Web体験版: ComfyUIクライアントは不要
...
def handle_input(user_input, chat_ph=None):
    ...
    try:
        ...
        # ==================================================
            ...
            # 5. ターン制限ロジック（Web体験版: 15ターン制限を強制適用）
            # Web体験版: IS_DEMO_MODEは強制的にTrue
            IS_DEMO_MODE = True
            
            # Web体験版: 15ターン制限（14回目でエンディング、15回目でHPリンク）
            if IS_DEMO_MODE:
...
""",unsafe_allow_html=True)
    ...
    with col_left:
        # Web体験版バッジを左カラム上部に表示（クリック可能なリンク）
        # Web体験版: IS_DEMO_MODEは強制的にTrue
        IS_DEMO_MODE = True
        DEMO_HP_URL = "https://x.com/MugenH50915"
        if IS_DEMO_MODE:
    ...
    with col_center:
        ...
        # Web体験版: 15ターン制限チェック または ゲーム終了フラグ
        # Web体験版: IS_DEMO_MODEは強制的にTrue
        IS_DEMO_MODE = True
        DEMO_HP_URL = "https://x.com/MugenH50915"
                ...
                if st.button(lang_mgr.get("text_0068", "🖼️ シーン画像を生成"), key="btn_gen_scene_right", width="stretch"):
                 try:
                    ...
                    if not target_h:
                        ...
                        # Web体験版: ComfyUI画像生成を無効化
                        st.info("体験版ではシーン画像生成機能は利用できません。事前画像のみ表示されます。")
                        # 3. 生成実行（Web体験版では無効化）
...
# Web体験版: render_age_gate() は削除されました
...
# Web体験版: save_settings() は削除されました
# Web体験版: get_available_gemini_models() は削除されました

# Web体験版: render_age_gate() (2つ目) は削除されました
...
def render_title_screen():
    ...
    with st.container():
        ...
        with c_load:
            # Web体験版: LOADボタンを削除（体験版では利用不可）
            # 📂 LOAD (Web体験版では無効化)
            st.button("📂 LOAD (体験版では利用できません)", key="btn_title_load", width="stretch", disabled=True) 
        ...
        if IS_DEMO_MODE:
            ...
            with st.spinner(lang_mgr.get("text_0109", "運命の相手を探しています...（設定生成・立ち絵描画）")):
                try:
                    ...
                    # Web体験版: ComfyUI画像生成を無効化（体験版では事前画像のみ）
                    ...
                    # 体験版では事前画像を使用（demo_heroineフォルダから読み込み済み）
                    pass
        ...
        with col_img:
            ...
            with st.expander(lang_mgr.get("text_0115", "🎨 立ち絵を調整・再描画"), expanded=False):
                ...
                if st.button(lang_mgr.get("text_0116", "🔄 再生成 (タグ追加)"), width="stretch"):
                    ...
                    # Web体験版: ComfyUI画像生成を無効化
                    st.info("体験版では画像再生成機能は利用できません。事前画像のみ表示されます。")
            ...
            if IS_DEMO_MODE:
                ...
                try:
                    ...
                    st.session_state.intro_text = default_intro
                        ...
                        # Web体験版: 導入文に合わせて場所を設定（雑居ビルの非常階段）
                st.session_state.current_location = {
                    "base_id": "08_DUNGEON",
